...
# cell predictions
interDSCHan = intersect(library180, rownames(meth.data))
...
CellPredsDSCHan180 = data.frame(projectWBCnew(HannumDSC, HanmeanmethDSC, Lwbc)*100)
save(CellPredsDSCHan180, file= "Cell Preds DSC Hannum 180.RData")

# compute the multivariate R2
meth.data.sub = meth.data
XDSCHan = model.matrix(~ CD8T + CD4T + NK + Bcell + Gran, data = CellPredsDSCHan180)

Llist = list()
Llist[[1]] = c(1,0,0,0,0,0)
MultiDSCHan180 = multiRegMultistat(meth.data.sub, XDSCHan, Llist)

save(MultiDSCHan180, file = "Hannum modifed DSC Results 180.RData")
...
interlegHan = intersect(tstats180, rownames(meth.data))
...
Lwbc = diag(6)
colnames(Lwbc) = rownames(Lwbc) = cellTypes
CellPredslegHan180 = data.frame(projectWBCnew(HannumLeg, Hanmeanmethleg, Lwbc)*100)
save(CellPredslegHan180, file= "Cell Preds Legacy Hannum 180.RData")

# compute the multivariate R2
meth.data.sub = meth.data
Xleghan = model.matrix(~ CD8T + CD4T + NK + Bcell + Gran, data = CellPredslegHan180)

Llist = list()
Llist[[1]] = c(1,0,0,0,0,0)
MultilegHan180 = multiRegMultistat(meth.data.sub, Xleghan, Llist)

save(MultilegHan180, file = "Hannum Legacy Results 180.RData")