...
DSCOptimFein <- dat[library240,]

#subset mean methylation matrix to our library
DSCmeanmethFein <- meanmeth[library240,]

#get cell proportion estimates
cellTypes = colnames(DSCmeanmethFein)
Lwbc = diag(6)
colnames(Lwbc) = rownames(Lwbc) = cellTypes
CellPredsDSCFein240 = data.frame(projectWBCnew(DSCOptimFein, DSCmeanmethFein , Lwbc)*100)
save(CellPredsDSCFein240, file= "Cell Preds DSC Feinberg 240.RData")

#get design matrix for regression
XDSCFein = model.matrix(~ CD8T + CD4T + NK + Bcell + Gran, data = CellPredsDSCFein240)

#compute the multivariate R2
Llist = list()
Llist[[1]] = c(1,0,0,0,0,0)
MultiDSCFein240 = multiRegMultistat(dat, XDSCFein, Llist)

save(MultiDSCFein240, file = "Feinberg Modified DSC Results 240.RData")
...
#subset feinberg data to our library
legacyOptimFein <- dat[tstats240,]

#subset mean methylation matrix to our library
legmeanmethFein <- meanmeth[tstats240,]

#get cell proportion estimates
cellTypes = colnames(legmeanmethFein)
Lwbc = diag(6)
colnames(Lwbc) = rownames(Lwbc) = cellTypes
CellPredslegFein240 = data.frame(projectWBCnew(legacyOptimFein, legmeanmethFein, Lwbc)*100)
save(CellPredslegFein240, file= "Cell Preds Legacy Feinberg 240.RData")

#get design matrix for regression
XLegFein = model.matrix(~ CD8T + CD4T + NK + Bcell + Gran, data = CellPredslegFein240)

# compute the multivariate R2
Llist = list()
Llist[[1]] = c(1,0,0,0,0,0)
MultiLegFein240 = multiRegMultistat(dat, XLegFein, Llist)

save(MultiLegFein240, file = "Feinberg Legacy Results 240.RData")
