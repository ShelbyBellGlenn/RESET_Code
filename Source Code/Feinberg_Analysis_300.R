...
DSCOptimFein <- dat[library300,]

#subset mean methylation matrix to our library
DSCmeanmethFein <- meanmeth[library300,]

#get cell proportion estimates
cellTypes = colnames(DSCmeanmethFein)
Lwbc = diag(6)
colnames(Lwbc) = rownames(Lwbc) = cellTypes
CellPredsDSCFein300 = data.frame(projectWBCnew(DSCOptimFein, DSCmeanmethFein , Lwbc)*100)
save(CellPredsDSCFein300, file= "Cell Preds DSC Feinberg 300.RData")

#get design matrix for regression
XDSCFein = model.matrix(~ CD8T + CD4T + NK + Bcell + Gran, data = CellPredsDSCFein300)

#compute the multivariate R2
Llist = list()
Llist[[1]] = c(1,0,0,0,0,0)
MultiDSCFein300 = multiRegMultistat(dat, XDSCFein, Llist)

save(MultiDSCFein300, file = "Feinberg Modified DSC Results 300.RData")
...
#subset feinberg data to our library
legacyOptimFein <- dat[tstats300,]

#subset mean methylation matrix to our library
legmeanmethFein <- meanmeth[tstats300,]

#get cell proportion estimates
cellTypes = colnames(legmeanmethFein)
Lwbc = diag(6)
colnames(Lwbc) = rownames(Lwbc) = cellTypes
CellPredslegFein300 = data.frame(projectWBCnew(legacyOptimFein, legmeanmethFein, Lwbc)*100)
save(CellPredslegFein300, file= "Cell Preds Legacy Feinberg 300.RData")

#get design matrix for regression
XLegFein = model.matrix(~ CD8T + CD4T + NK + Bcell + Gran, data = CellPredslegFein300)

# compute the multivariate R2
Llist = list()
Llist[[1]] = c(1,0,0,0,0,0)
MultiLegFein300 = multiRegMultistat(dat, XLegFein, Llist)

save(MultiLegFein300, file = "Feinberg Legacy Results 300.RData")
