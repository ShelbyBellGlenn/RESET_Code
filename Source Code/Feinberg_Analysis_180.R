...
DSCOptimFein <- dat[library180,]

#subset mean methylation matrix to our library
DSCmeanmethFein <- meanmeth[library180,]

#get cell proportion estimates
cellTypes = colnames(DSCmeanmethFein)
Lwbc = diag(6)
colnames(Lwbc) = rownames(Lwbc) = cellTypes
CellPredsDSCFein180 = data.frame(projectWBCnew(DSCOptimFein, DSCmeanmethFein , Lwbc)*100)
save(CellPredsDSCFein180, file= "Cell Preds DSC Feinberg 180.RData")

#get design matrix for regression
XDSCFein = model.matrix(~ CD8T + CD4T + NK + Bcell + Gran, data = CellPredsDSCFein180)

#compute the multivariate R2
Llist = list()
Llist[[1]] = c(1,0,0,0,0,0)
MultiDSCFein180 = multiRegMultistat(dat, XDSCFein, Llist)

save(MultiDSCFein180, file = "Feinberg Modified DSC Results 180.RData")
...
#subset feinberg data to our library
legacyOptimFein <- dat[tstats180,]

#subset mean methylation matrix to our library
legmeanmethFein <- meanmeth[tstats180,]

#get cell proportion estimates
cellTypes = colnames(legmeanmethFein)
Lwbc = diag(6)
colnames(Lwbc) = rownames(Lwbc) = cellTypes
CellPredslegFein180 = data.frame(projectWBCnew(legacyOptimFein, legmeanmethFein, Lwbc)*100)
save(CellPredslegFein180, file= "Cell Preds Legacy Feinberg 180.RData")

#get design matrix for regression
XLegFein = model.matrix(~ CD8T + CD4T + NK + Bcell + Gran, data = CellPredslegFein180)

# compute the multivariate R2
Llist = list()
Llist[[1]] = c(1,0,0,0,0,0)
MultiLegFein180 = multiRegMultistat(dat, XLegFein, Llist)

save(MultiLegFein180, file = "Feinberg Legacy Results 180.RData")
