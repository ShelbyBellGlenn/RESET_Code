...
DSCOptimFein <- dat[library540,]

#subset mean methylation matrix to our library
DSCmeanmethFein <- meanmeth[library540,]

#get cell proportion estimates
cellTypes = colnames(DSCmeanmethFein)
Lwbc = diag(6)
colnames(Lwbc) = rownames(Lwbc) = cellTypes
CellPredsDSCFein540 = data.frame(projectWBCnew(DSCOptimFein, DSCmeanmethFein , Lwbc)*100)
save(CellPredsDSCFein540, file= "Cell Preds DSC Feinberg 540.RData")

#get design matrix for regression
XDSCFein = model.matrix(~ CD8T + CD4T + NK + Bcell + Gran, data = CellPredsDSCFein540)

#compute the multivariate R2
Llist = list()
Llist[[1]] = c(1,0,0,0,0,0)
MultiDSCFein540 = multiRegMultistat(dat, XDSCFein, Llist)

save(MultiDSCFein540, file = "Feinberg Modified DSC Results 540.RData")
...
#subset feinberg data to our library
legacyOptimFein <- dat[tstats540,]

#subset mean methylation matrix to our library
legmeanmethFein <- meanmeth[tstats540,]

#get cell proportion estimates
cellTypes = colnames(legmeanmethFein)
Lwbc = diag(6)
colnames(Lwbc) = rownames(Lwbc) = cellTypes
CellPredslegFein540 = data.frame(projectWBCnew(legacyOptimFein, legmeanmethFein, Lwbc)*100)
save(CellPredslegFein540, file= "Cell Preds Legacy Feinberg 540.RData")

#get design matrix for regression
XLegFein = model.matrix(~ CD8T + CD4T + NK + Bcell + Gran, data = CellPredslegFein540)

# compute the multivariate R2
Llist = list()
Llist[[1]] = c(1,0,0,0,0,0)
MultiLegFein540 = multiRegMultistat(dat, XLegFein, Llist)

save(MultiLegFein540, file = "Feinberg Legacy Results 540.RData")
