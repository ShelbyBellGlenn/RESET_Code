...
# cell predictions
interDSCHan = intersect(library540, rownames(meth.data))
...
CellPredsDSCHan540 = data.frame(projectWBCnew(HannumDSC, HanmeanmethDSC, Lwbc)*100)
save(CellPredsDSCHan540, file= "Cell Preds DSC Hannum 540.RData")

# compute the multivariate R2
meth.data.sub = meth.data
XDSCHan = model.matrix(~ CD8T + CD4T + NK + Bcell + Gran, data = CellPredsDSCHan540)

Llist = list()
Llist[[1]] = c(1,0,0,0,0,0)
MultiDSCHan540 = multiRegMultistat(meth.data.sub, XDSCHan, Llist)

save(MultiDSCHan540, file = "Hannum modifed DSC Results 540.RData")
...
interlegHan = intersect(tstats540, rownames(meth.data))
...
Lwbc = diag(6)
colnames(Lwbc) = rownames(Lwbc) = cellTypes
CellPredslegHan540 = data.frame(projectWBCnew(HannumLeg, Hanmeanmethleg, Lwbc)*100)
save(CellPredslegHan540, file= "Cell Preds Legacy Hannum 540.RData")

# compute the multivariate R2
meth.data.sub = meth.data
Xleghan = model.matrix(~ CD8T + CD4T + NK + Bcell + Gran, data = CellPredslegHan540)

Llist = list()
Llist[[1]] = c(1,0,0,0,0,0)
MultilegHan540 = multiRegMultistat(meth.data.sub, Xleghan, Llist)

save(MultilegHan540, file = "Hannum Legacy Results 540.RData")